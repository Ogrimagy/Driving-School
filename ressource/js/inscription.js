		...
		if(!pattern.test(mdp)){
			...
			if(mdpC != ""){
				if(!pattern.test(mdpC)){
					document.getElementById("err_cpw").textContent = "Le mot de passe contient au moins 4 caractères alpha-numériques('_' est autorisé)";
				...
				document.getElementById("err_cpw").textContent = "La confirmation du mot de passe doit être renseignée";
				progression(7, -15);
			}
		}
...
function validateForm(){
	...
	if(nom == "" || prenom == "" || age<18 || ville == "" || ad_email == "" || phone == "" || pw == "" || cpw == "" || pw != cpw || 
		(!nomPattern.test(nom)) || (!nomPattern.test(prenom)) || (!villePattern.test(ville)) || (!mailPattern.test(ad_email)) || 
		(!telPattern.test(phone)) || (!mdpPattern.test(pw)) || (!mdpPattern.test(cpw))){
