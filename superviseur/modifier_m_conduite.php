...
<?php
    ...
    if(isset($_POST['modifier_m_cond'])) {
        $id_s=$_POST['id_s'];
...
<head>
    ...
    <script>
        function afficherDivMod() {
            setTimeout('document.getElementById("divModz").style.opacity = "1"', 1);
            document.getElementById("divModz").style.opacity = "0.5";
            document.getElementById("divModz").style.display = "block";
        }
    </script>
</head>
                ...
                <table class="table" width="100%">
                    ...
                    <?php 
                        if(isset($_POST['modifier_m_cond'])) {
                            ...
                            $resultat=$connexion->query("SELECT * FROM `employee` WHERE id='$id_cond'");
                            while ($row=$resultat->fetch()) {
                    ?>

                    <tr><td colspan="2"><label id="title"><h2>Modification :: <?php echo ucfirst($dpNom) . " " . ucfirst($dpPre) ?> </h2></label>
                            <div class="typeMod">Role: <span class="vMod">Moniteur Conduite</span></div></td></tr>
                            <tr></tr><tr></tr>
                    ...
                    <tr>
                        <td>
                            <select id="etat" name="etat">
                                ...
                                <option value="4" selected>Moniteur de conduite</option>
                            </select>
                        </td>
                    </tr>
                    <!-------------------------------------------------------------------------------->
                    ...
                    <?php
                            ...
                            if (isset($_POST['valider'])) {
                                ...
                                echo "<script>if(confirm(\"Votre moniteur de conduite: ".$nom." ".$prenom." est modifier avec succée.\")){document.location.href='superviseur.php'};</script>";
